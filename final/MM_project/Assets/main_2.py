...
def follow_trajectory( old_x, new_x, theta, phi, robot,  smoothness = 0.1):
    '''
    Naive implementation of linear smoothing trajectory
    '''
    diff = new_x - old_x
    print(diff)
...
def main():
    ...
    for phi in DESIRED_PHI:
        for theta in DESIRED_THETA:
            ...
            #follow_trajectory(prev_x, x, theta, phi, robot)
            q = ik(x, theta, phi)
            ...
            while True:
                ...
                try: 
                    ...
                    if np.linalg.norm(x - prev_x) / np.linalg.norm(x) < 0.01:
                        ...
                        break
                    follow_trajectory(prev_x, x, theta, phi, robot, smoothness = 0.003)
                    q = ik(x, theta, phi)
                    robot.command_robot(q[0:3], q[3:7], True)
                except:
